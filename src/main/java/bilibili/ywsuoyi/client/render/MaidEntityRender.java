package bilibili.ywsuoyi.client.render;
...
public class MaidEntityRender extends EntityRenderer<MaidEntity> {

    private static final Logger LOGGER = LogManager.getLogger();
    public MaidModel model=null;
    public Identifier texture = null;


    public MaidEntityRender(EntityRenderDispatcher dispatcher) {
        super(dispatcher);
        this.shadowRadius = 1;
    }
    ...
    public Identifier getTexture(MaidEntity entity) {
        if(this.texture==null){
            this.texture=ModelManager.maid_texture_list.get(new Identifier("touhou_little_maid","hakurei_reimu"));
        }
        return texture;
    }

    public void render(MaidEntity livingEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        matrixStack.push();
        if(this.model==null){
            this.model=new MaidModel(
                    ModelManager.maid_model_list.get(new Identifier("touhou_little_maid","hakurei_reimu")),
                    ModelManager.maid_animations_list.get(new Identifier("touhou_little_maid","hakurei_reimu"))
            );
        }
     ...
     */
    @Nullable
    public RenderLayer getRenderLayer(MaidEntity entity, boolean showBody, boolean translucent, boolean showOutline) {
    ...
    public boolean isVisible(MaidEntity entity) {
     ...
     */
    public boolean isShaking(MaidEntity entity) {
        return false;
    }

    public void setupTransforms(MaidEntity entity, MatrixStack matrices, float animationProgress, float bodyYaw, float tickDelta) {
    ...
    public float getHandSwingProgress(MaidEntity entity, float tickDelta) {
     ...
     */
    public float getAnimationProgress(MaidEntity entity, float tickDelta) {
        return (float) entity.age + tickDelta;
    }

    public float getLyingAngle(MaidEntity entity) {
        return 90.0F;
    }

    public float getAnimationCounter(MaidEntity entity, float tickDelta) {
        return 0.0F;
    }

    public void scale(MaidEntity entity, MatrixStack matrices, float amount) {
    }

    public boolean hasLabel(MaidEntity livingEntity) {
